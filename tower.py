...
class TowerGroup:
    ...
    def draw(self, win):
        ...
        # draw tower range
        if self.selected_tower is not None:             # here, for the description of sell
    ...
    def upgrade(self):
        ...
        for tw in self.__towers:            # select 3 towers
            if self.selected_tower == tw:   # if you click the tower you choose
                if tw.level < 5:            # Upgrade the level. However, the maximum level is 5. Set the limit of level
                    tw.level += 1           # level +1 upgrade
    ...
    def sell(self):
        ...
        for i in range(len(self.__towers)):              # select 3 towers
            if self.selected_tower == self.__towers[i]:  # if you click the tower you choose
                del self.__towers[i]                     # delete the tower you click
                self.selected_tower = None               # make self.selected_tower = None to not execute 
                                                         # self.selected_tower.draw_effect_range(win) and
                                                         # self.upgrade_menu.draw(win)
